(function(mod) {
  ...
  function displayOutline(cm) {
    var state = cm.state.angularOutline;
    if (!state) return;
  ...
  CodeMirror.defineOption("angularOutline", false, function(cm, val, old) {
    if (old && old != CodeMirror.Init) {
      cm.off("change", onChange);
    }
    if (val) {
      var state = cm.state.angularOutline = new OutlineState(cm, parseOptions(cm, val));
